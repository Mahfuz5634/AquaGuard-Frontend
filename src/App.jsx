...
const Dashboard = () => {
  ...
  // Historical Telemetry for the Graph
          ...
          <div className="flex flex-wrap items-center gap-3 w-full xl:w-auto">
            <div className="flex items-center gap-3 bg-white/[0.02] border border-white/5 px-4 py-2.5 rounded-xl backdrop-blur-md shadow-lg">
              ...
              <div className="flex flex-col">
                ...
                <span className="text-xs font-mono text-slate-300 font-bold leading-tight">
                  28ms
                </span>
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-8">
          ...
          <div className="bg-[#0b1221]/80 backdrop-blur-xl rounded-2xl p-5 border border-white/5 hover:border-cyan-500/30 hover:bg-[#0b1221] transition-all group relative overflow-hidden shadow-2xl">
            ...
            <div className="flex justify-between items-start mb-4 relative z-10">
              ...
              <div
                className={`flex items-center gap-1 text-xs font-bold px-2 py-1 rounded-md ${currentData.waterLevel.isUp ? "bg-emerald-500/10 text-emerald-400" : "bg-rose-500/10 text-rose-400"}`}
              >
                {currentData.waterLevel.isUp ? (
                  <TrendingUp size={14} />
                ) : (
                  <TrendingDown size={14} />
                )}
                {currentData.waterLevel.trend}
              </div>
            </div>
            <div className="relative z-10">
              <div className="flex items-center justify-between">
                <p className="text-slate-400 font-semibold text-xs uppercase tracking-widest mb-1">
                  Tank Capacity
                </p>
                <span className="text-[10px] font-mono text-cyan-500/70 border border-cyan-500/20 px-1.5 rounded bg-cyan-500/5">
                  Sonar: {currentData.waterLevel.distance}
                </span>
              </div>
            ...
            <div className="absolute bottom-0 left-0 h-1.5 bg-cyan-500/20 w-full">
              <div
                ...
                style={{ width: `${currentData.waterLevel.value}%` }}
              ></div>
            </div>
          </div>
          ...
          <div className="bg-[#0b1221]/80 backdrop-blur-xl rounded-2xl p-5 border border-white/5 hover:border-purple-500/30 hover:bg-[#0b1221] transition-all group relative overflow-hidden shadow-2xl">
            ...
            <div className="flex justify-between items-start mb-4 relative z-10">
              ...
              <div
                className={`flex items-center gap-1 text-xs font-bold px-2 py-1 rounded-md ${currentData.ph.isUp ? "bg-emerald-500/10 text-emerald-400" : "bg-rose-500/10 text-rose-400"}`}
              >
                {currentData.ph.isUp ? (
                  <TrendingUp size={14} />
                ) : (
          ...
          <div className="bg-[#0b1221]/80 backdrop-blur-xl rounded-2xl p-5 border border-white/5 hover:border-rose-500/30 hover:bg-[#0b1221] transition-all group relative overflow-hidden shadow-2xl">
            ...
            <div className="flex justify-between items-start mb-4 relative z-10">
              ...
              <div
                className={`flex items-center gap-1 text-xs font-bold px-2 py-1 rounded-md ${currentData.temp.isUp ? "bg-rose-500/10 text-rose-400" : "bg-blue-500/10 text-blue-400"}`}
              >
                {currentData.temp.isUp ? (
                  <TrendingUp size={14} />
                ) : (
                  <TrendingDown size={14} />
                )}
                {currentData.temp.trend}
              </div>
            </div>
            <div className="relative z-10">
              <p className="text-slate-400 font-semibold text-xs uppercase tracking-widest mb-1">
          ...
          <div className="bg-[#0b1221]/80 backdrop-blur-xl rounded-2xl p-5 border border-white/5 hover:border-blue-500/30 hover:bg-[#0b1221] transition-all group relative overflow-hidden shadow-2xl">
            ...
            <div className="flex justify-between items-start mb-4 relative z-10">
              ...
              <div
                className={`flex items-center gap-1 text-xs font-bold px-2 py-1 rounded-md ${currentData.tds.isUp ? "bg-rose-500/10 text-rose-400" : "bg-emerald-500/10 text-emerald-400"}`}
              >
                {currentData.tds.isUp ? (
                  <TrendingUp size={14} />
                ) : (
                  <TrendingDown size={14} />
                )}
                {currentData.tds.trend}
              </div>
            </div>
            <div className="relative z-10">
              <p className="text-slate-400 font-semibold text-xs uppercase tracking-widest mb-1">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
              <div>
                <h3 className="text-xl font-bold text-white tracking-wide flex items-center gap-2">
                  <Activity size={20} className="text-blue-400" /> Sensor
                  Telemetry Map
                </h3>
              </div>
              <div className="flex bg-black/40 rounded-lg p-1 border border-white/5">
                <button className="px-5 py-1.5 text-xs font-bold bg-blue-500/20 text-blue-400 rounded-md border border-blue-500/20 shadow-[0_0_10px_rgba(59,130,246,0.1)]">
                  Live
                </button>
                <button className="px-5 py-1.5 text-xs font-bold text-slate-400 hover:text-white transition-colors">
                  1H
                </button>
                <button className="px-5 py-1.5 text-xs font-bold text-slate-400 hover:text-white transition-colors">
                  24H
          ...
          <div className="bg-[#0b1221]/80 backdrop-blur-xl rounded-3xl p-6 sm:p-8 border border-white/5 flex flex-col shadow-2xl relative">
            ...
            <div className="space-y-4 flex-grow z-10">
              {/* Aerator Relay */}
              <div
                className={`relative p-5 rounded-2xl border transition-all duration-300 ${relays.aerator ? "bg-cyan-500/5 border-cyan-500/30" : "bg-black/20 border-white/5"}`}
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    <div
                      className={`p-3 rounded-xl transition-all duration-300 ${relays.aerator ? "bg-cyan-500 text-white shadow-[0_0_20px_rgba(34,211,238,0.4)]" : "bg-white/5 text-slate-400"}`}
                    >
                      <Wind
                        size={20}
                        className={relays.aerator ? "animate-spin-slow" : ""}
                      />
                    </div>
                    <div>
                  ...
                  <button
                    onClick={() => toggleRelay("aerator")}
                    className={`relative inline-flex h-6 w-12 items-center rounded-full transition-colors duration-300 focus:outline-none ${relays.aerator ? "bg-cyan-500 shadow-[0_0_10px_rgba(34,211,238,0.4)]" : "bg-white/10"}`}
                  >
                    <span
                      className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform duration-300 ${relays.aerator ? "translate-x-7" : "translate-x-1"}`}
                    />
                  </button>
                </div>
              </div>
              ...
              {/* Pump Relay */}
              <div
                className={`relative p-5 rounded-2xl border transition-all duration-300 ${relays.waterPump ? "bg-blue-500/5 border-blue-500/30" : "bg-black/20 border-white/5"}`}
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    <div
                      className={`p-3 rounded-xl transition-all duration-300 ${relays.waterPump ? "bg-blue-500 text-white shadow-[0_0_20px_rgba(59,130,246,0.4)]" : "bg-white/5 text-slate-400"}`}
                    >
                      <Power size={20} />
                    </div>
                    <div>
                  ...
                  <button
                    onClick={() => toggleRelay("waterPump")}
                    className={`relative inline-flex h-6 w-12 items-center rounded-full transition-colors duration-300 focus:outline-none ${relays.waterPump ? "bg-blue-500 shadow-[0_0_10px_rgba(59,130,246,0.4)]" : "bg-white/10"}`}
                  >
                    <span
                      className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform duration-300 ${relays.waterPump ? "translate-x-7" : "translate-x-1"}`}
                    />
                  </button>
                </div>
              </div>
            ...
            {/* Smart Feeder Control with Hopper Status */}
            <div className="mt-6 pt-6 border-t border-white/10 z-10">
              ...
              <div className="w-full bg-black/40 rounded-full h-2.5 mb-5 border border-white/5">
                <div
                  ...
                  style={{ width: `${currentData.feedHopper}%` }}
                ></div>
              </div>
